...
@if(in_array(session('firebase_role', 'guest'), ['admin','head_women']))
    <a href="{{ route('surgery.head_women') }}" class="head-btn">
...
<div class="body-wrap">
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      <a href="{{ route('daily-meds') }}" class="nav-btn active">
...
<div class="modal-overlay" id="headModal">
  ...
  <div class="modal-box">
    ...
    <form method="POST" action="{{ route('login') }}" id="headForm" novalidate>
      ...
      <input type="hidden" name="redirect_to" value="{{ route('surgery.women') }}">
