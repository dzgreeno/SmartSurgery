...
window.saveFiche = async function(){
  ...
  .forEach(el=>{
    ...
    if(label){
      data[label.innerText.replace(/[.#$/\[\]]/g, '_')] = el.value;
    }
  ...
  // جمع بيانات جدول حركة المريض
  const movements = [];
  document.querySelectorAll('#ficheMain .mv tbody tr').forEach(row => {
    const inputs = row.querySelectorAll('input');
    let hasData = false;
    const rowData = {};
    inputs.forEach((inp, idx) => {
      const cols = ['service', 'date', 'heure', 'salle_lit', 'medecin'];
      if (inp.value.trim()) hasData = true;
      rowData[cols[idx] || `col_${idx}`] = inp.value;
    });
    if (hasData) movements.push(rowData);
  });

  try{

    const newRef = push(ref(db, 'fiche_navettes'));
    await set(newRef, {
    ...
    showToast("✅ تم حفظ Fiche Navette في قاعدة البيانات");
